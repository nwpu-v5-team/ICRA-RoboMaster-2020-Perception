...
class MobileNetV2(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(len(laterals)-1, 0 ,-1):
            laterals[i-1] += nn.functional.interpolate(laterals[i],size=(int(laterals[i-1].shape[2]), int(laterals[i-1].shape[3])), mode="nearest")

        for i in range(len(laterals)):
            laterals[i] = self.fpn_list[i](laterals[i])
        # for f in laterals:
        #      print(f.shape)
        for i in range(3):
...
@registry.BackBone.register('mobileNet-V2')
def mobilenet_v2():
    model = MobileNetV2()
    #if pretrained:
